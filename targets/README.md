...
- **OpenWrt / 内核口径（含 `hiker_x9-standard` 与其它 X9 profile）**：默认 CI 见 [`.github/workflows/openwrt-builder.yml`](../.github/workflows/openwrt-builder.yml)：**`REPO_BRANCH=openwrt-24.10`**、源码 **`https://github.com/openwrt/openwrt`**，即官方 **24.10 稳定系列**（刷机后 `/etc/openwrt_release` 一般为 **`24.10.x`**，**不是** **SNAPSHOT** / **master**），**不是** OEM 的 **Chaos Calmer 15.05**。内核与上游 **`target/linux/ramips`** 一致；该分支上 [`Makefile`](https://github.com/openwrt/openwrt/blob/openwrt-24.10/target/linux/ramips/Makefile) 当前为 **`KERNEL_PATCHVER:=6.6`**（**精确 `6.6.x`** 以你那次编译的 tag/commit 为准）。若要换 **25.12** 等新稳定系列，改 workflow 里 **`REPO_BRANCH`** 并在本地试编确认 overlay 无 API 差异即可。
