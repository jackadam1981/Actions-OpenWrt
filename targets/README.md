...
- **OpenWrt / 内核口径（含 `hiker_x9-standard` 与其它 X9 profile）**：默认 CI 见 [`.github/workflows/openwrt-builder.yml`](../.github/workflows/openwrt-builder.yml)：**`REPO_BRANCH=openwrt-25.12`**（官方 **25.12 稳定系列分支**，上游发布 **25.12.3 / 25.12.4** 等会推进该分支 tip，**无需每次改 workflow 小版本号**）；配合 **`OPENWRT_FETCH_DEPTH`**（默认 **120**）做浅克隆/拉取，让 **`git describe`** 能见到近期 **`v25.12.*`** 标签，**减轻** **`SNAPSHOT r0-<hash>`** / **`LuCI Master`**。若某次构建仍显示 SNAPSHOT，可再**加大 depth** 或临时把 **`REPO_BRANCH`** 设为精确 **`v25.12.x`** 钉死。源码 **`https://github.com/openwrt/openwrt`**；**不是** OEM **Chaos Calmer 15.05**。内核与上游 **`target/linux/ramips`** 一致（[`Makefile`](https://github.com/openwrt/openwrt/blob/openwrt-25.12/target/linux/ramips/Makefile) 上 **`KERNEL_PATCHVER:=6.12`**，精确 **`6.12.x`** 以当次 checkout 为准）。换 **26.xx** 时改 **`REPO_BRANCH`**（及必要时 **depth**）并核对 overlay。
