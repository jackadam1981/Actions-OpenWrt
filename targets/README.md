...
- **OpenWrt / 内核口径（含 `hiker_x9-standard` 与其它 X9 profile）**：默认 CI 见 [`.github/workflows/openwrt-builder.yml`](../.github/workflows/openwrt-builder.yml)：**`REPO_BRANCH=v25.12.2`**（**带 tag 的 ref**，与浅克隆配合后 **`/etc/openwrt_release`** 为 **`25.12.2`** 一类稳定串，**避免** **`SNAPSHOT r0-<hash>`** / **`LuCI Master`**）；源码 **`https://github.com/openwrt/openwrt`**。**不是** OEM **Chaos Calmer 15.05**。内核与上游 **`target/linux/ramips`** 一致；[`Makefile`](https://github.com/openwrt/openwrt/blob/v25.12.2/target/linux/ramips/Makefile) 上 **`KERNEL_PATCHVER:=6.12`**（精确 **`6.12.x`** 以该 tag 为准）。若要跟 **25.12 分支最新 tip** 而非固定小版本，可改 **`REPO_BRANCH=openwrt-25.12`** 并加大 **`git clone`/`fetch` depth**（否则仍易 **SNAPSHOT**）。换 **26.xx** 等新系列时改 **`REPO_BRANCH`** 并核对 overlay。
