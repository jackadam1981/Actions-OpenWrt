...
- **与本仓固件对照**：若要在 **OpenWrt 稳定版（CI 默认 `openwrt-25.12`）** 上追求「全功能路由 + LuCI + WiFi」取向，见 **`hiker_x9-standard`**（`targets/hiker-x9/target/linux/ramips/image/hiker.mk`），其设计说明见 [`targets/README.md`](../../targets/README.md)（**非**本快照包名的逐条移植）。**当前 `standard` 为排查首启未编入 `hiker-x9-standard-defaults`，LAN 多为上游默认（如 `192.168.1.1`）**；恢复 **`192.168.100.1`** 对齐 **`minimal`** 需待 `hiker.mk` 再次拉入该 defaults 包。OEM 采集机仍可能是 **`192.168.168.1`**。
