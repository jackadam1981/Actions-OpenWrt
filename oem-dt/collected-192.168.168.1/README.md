...
- **与本仓固件对照**：若要在 **OpenWrt 稳定版（CI 默认 `openwrt-24.10`）** 上追求「全功能路由 + LuCI + WiFi」取向，见 **`hiker_x9-standard`**（`targets/hiker-x9/target/linux/ramips/image/hiker.mk`），其设计说明见 [`targets/README.md`](../../targets/README.md)（**非**本快照包名的逐条移植）。**标准版 LAN 已与 `minimal` 对齐为 `192.168.100.1`**（OEM 采集机仍可能是 `192.168.168.1`）。
